...
class PromptBuilder:
    ...
    def add_file_content_to_prompt(self, main_window):
        ...
        if len(selected_items) > 1:
            self.warning_message.message_box("Warning", "Select only one item at a time")
            return
        ...
        if os.path.isfile(file_path):
            ...
            if file_content:
                if main_window.actionLine_Enumerator.isChecked():
                    self._add_file_content_with_line_numbers(main_window.tedit_tab5, file_path, file_content)
                else:
                    self._add_file_content_without_line_numbers(main_window.tedit_tab5, file_path, file_content)
        elif os.path.isdir(file_path):
            # Process all files in the folder
            for root, _, files in os.walk(file_path):
                for file_name in files:
                    full_file_path = os.path.join(root, file_name)
                    file_content = main_window.file_handler.read_file_content(full_file_path)
                    if file_content:
                        if main_window.actionLine_Enumerator.isChecked():
                            self._add_file_content_with_line_numbers(main_window.tedit_tab5, full_file_path, file_content, silent=True)
                        else:
                            self._add_file_content_without_line_numbers(main_window.tedit_tab5, full_file_path, file_content, silent=True)
        else:
            self.warning_message.message_box("Warning", "Something went wrong!")

    def _add_file_content_with_line_numbers(self, text_edit:QPlainTextEdit, file_path, file_content, silent=True):
        numbered_prompt = ""
        for i, line in enumerate(file_content.splitlines()):
           numbered_prompt += f"{i+1}: {line}\n"
        text_edit.appendPlainText(f"<file name=\"{os.path.basename(file_path)}\">\n{numbered_prompt}</file>")
        if not silent:
            self.warning_message.message_box("Success", f"File content from {os.path.basename(file_path)} added to Files tab")

    def _add_file_content_without_line_numbers(self, text_edit:QPlainTextEdit, file_path, file_content, silent=True):
        text_edit.appendPlainText(f"<file name=\"{os.path.basename(file_path)}\">\n{file_content}</file>")
        if not silent:
            self.warning_message.message_box("Success", f"File content from {os.path.basename(file_path)} added to Files tab")
    ...
    def add_folder_files_content_to_prompt(self, main_window, folder_path):
        ...
        for root, _, files in os.walk(folder_path):
            for file_name in files:
                ...
                if file_content:
                    if main_window.actionLine_Enumerator.isChecked():
                        self._add_file_content_with_line_numbers(main_window.tab_context, full_file_path, file_content, silent=True)
                    else:
                        self._add_file_content_without_line_numbers(main_window.tab_context, full_file_path, file_content, silent=True)
    ...
    def add_files_to_context(self, main_window, file_paths):
        if file_paths:
            for file_path in file_paths:
                 file_content = main_window.file_handler.read_file_content(file_path)
                 if file_content:
                     main_window.tab_context.appendPlainText(f"<file name=\"{os.path.basename(file_path)}\">\n{file_content}</file>")
    ...
    def _add_all_files_recursive(self, parent, main_window):
         ...
         for i in range(parent.childCount()):
            ...
            if os.path.isfile(file_path):
                if not file_path.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.webp', '.svg', '.ico', '.mp4', '.mov', '.avi', '.mkv', '.mp3', '.wav', '.ogg')):
                    file_content = main_window.file_handler.read_file_content(file_path)
                    if file_content:
                        if main_window.actionLine_Enumerator.isChecked():
                           self._add_file_content_with_line_numbers(main_window, file_path, file_content, silent=True)
                        else:
                            self._add_file_content_without_line_numbers(main_window, file_path, file_content, silent=True)
            elif os.path.isdir(file_path):
